...
# ssm155基于Java的公务员培训机构管理系统

![picture](https://raw.githubusercontent.com/GraduationProject-springboot/.github/main/img/wx.png)

### 点击播放视频 ▼
[![Watch the video](https://i.sstatic.net/Vp2cE.png)](https://www.bilibili.com/video/BV1T48XecE9G?p=150)


# 绪论
## 1.1 研究背景
当前社会各行业领域竞争压力非常大，随着当前时代的信息化，科学化发展，让社会各行业领域都争相使用新的信息技术，对行业内的各种相关数据进行科学化，规范化管理。这样的大环境让那些止步不前，不接受信息改革带来的信息技术的企业随时面临被淘汰，被取代的风险。所以当今，各个行业领域，不管是传统的教育行业，餐饮行业，还是旅游行业，医疗行业等领域都将使用新的信息技术进行信息革命，改变传统的纸质化，需要人手工处理工作事务的办公环境。软件信息技术能够覆盖社会各行业领域是时代的发展要求，各种数据以及文件真正实现电子化是信息社会发展的不可逆转的必然趋势。本公务员培训机构管理系统也是紧跟科学技术的发展，运用当今一流的软件技术实现软件系统的开发，让医生管理信息完全通过管理系统实现科学化，规范化，程序化管理。从而帮助信息管理者节省事务处理的时间，降低数据处理的错误率，对于基础数据的管理水平可以起到促进作用，也从一定程度上对随意的业务管理工作进行了避免，同时，公务员培训机构管理系统的数据库里面存储的各种动态信息，也为上层管理人员作出重大决策提供了大量的事实依据。总之，公务员培训机构管理系统是一款可以真正提升管理者的办公效率的软件系统。
## 1.2 目的和意义
信息数据的处理完全依赖人工进行操作，会耗费大量的人工成本，特别是面对大量的数据信息时，传统人工操作不仅不能对数据的出错率进行保证，还容易出现各种信息资源的低利用率与低安全性问题。更有甚者，耽误大量的宝贵时间，尤其是对信息的更新，归纳与统计更是耗财耗力的过程。所以电子化信息管理的出现就能缓解以及改变传统人工方式面临的处境，一方面可以确保信息数据在短时间被高效处理，还能节省人力成本，另一方面可以确保信息数据的安全性，可靠性，并可以实现信息数据的快速检索与修改操作，这些优点是之前的旧操作模式无法比拟的。因此公务员培训机构管理系统为数据信息的管理模式的升级与改革提供了重要的窗口。
## 1.3 论文结构安排
...
# 4 系统设计
公务员培训机构管理系统的设计方案比如功能框架的设计，比如数据库的设计的好坏也就决定了该系统在开发层面是否高效，以及在系统维护层面是否容易维护和升级，因为在系统实现阶段是需要考虑用户的所有需求，要是在设计阶段没有经过全方位考虑，那么系统实现的部分也就无从下手，所以系统设计部分也是至关重要的一个环节，只有根据用户需求进行细致全面的考虑，才有希望开发出功能健全稳定的程序软件。
## 4.1 系统概要设计
...
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|2|beikaokecheng\_name|String|课程名称|是|
|3|beikaokecheng\_video|String|预览视频|是|
|4|beikaokecheng\_photo|String|课程图片|是|
|5|beikaokecheng\_money|String|课程价格|是|
|6|jiangshi\_id|Integer|讲师|是|
|7|beikaokecheng\_content|String|课程详情|是|
...
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|9|yonghu\_id|Integer|预定人|是|
|10|beikaokecheng\_id|Integer|预定课程|是|
|11|insert\_time|Date|预定时间|是|
...
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|13|dic\_code|String|字段|是|
|14|dic\_name|String|字段名|是|
|15|code\_index|Integer|编码|是|
|16|index\_name|String|编码名字|是|
|17|super\_id|Integer|父字段id|是|
...
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|19|gongwuyuan\_name|String|公务员姓名|是|
|20|sex\_types|Integer|性别|是|
|21|gongwuyuan\_id\_number|String|身份证号|是|
|22|gongwuyuan\_phone|String|手机号|是|
|23|gongwuyuan\_photo|String|照片|是|
|24|gongwuyuan\_types|Integer|单位|是|
|25|gongwuyuan\_new\_money|BigDecimal|余额|是|
|26|create\_time|Date|创建时间|是|
表4.5讲师表

|序号|列名|数据类型|说明|允许空|
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|27|jiangshi\_name|String|讲师名称|是|
|28|sex\_types|Integer|讲师性别|是|
|29|jiangshi\_photo|String|讲师头像|是|
|30|jiangshi\_phone|String|讲师联系方式|是|
|31|jiangshi\_money|BigDecimal|讲师薪资|是|
|32|jiangshi\_content|String|讲师详情|是|
|33|create\_time|Date|创建时间|是|
表4.6公告表

|序号|列名|数据类型|说明|允许空|
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|34|news\_name|String|公告名称|是|
|35|news\_types|Integer|公告类型|是|
|36|news\_photo|String|公告图片|是|
|37|insert\_time|Date|公告时间|是|
|38|news\_content|String|公告详情|是|
...
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|40|yonghu\_name|String|考生姓名|是|
|41|sex\_types|Integer|性别|是|
|42|yonghu\_id\_number|String|身份证号|是|
|43|yonghu\_phone|String|手机号|是|
|44|yonghu\_photo|String|照片|是|
|45|yonghu\_xuexiao|String|学校|是|
|46|yonghu\_zhuanye|String|专业|是|
|47|yonghu\_new\_money|BigDecimal|余额|是|
|48|create\_time|Date|创建时间|是|
表4.8在职培训课程表

|序号|列名|数据类型|说明|允许空|
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|49|zaizhikecheng\_name|String|课程名称|是|
|50|zaizhikecheng\_video|String|预览视频|是|
|51|zaizhikecheng\_types|Integer|课程类型|是|
|52|zaizhikecheng\_photo|String|课程图片|是|
|53|zaizhikecheng\_money|String|课程价格|是|
|54|jiangshi\_id|Integer|讲师|是|
|55|zaizhikecheng\_content|String|课程详情|是|
...
| :-: | :-: | :-: | :-: | :-: |
|1|Id|Int|id|否|
|57|yonghu\_id|Integer|预定人|是|
|58|zaizhikecheng\_id|Integer|预定课程|是|
|59|insert\_time|Date|预定时间|是|
|60|create\_time|Date|创建时间|是|
表4.10用户表表
...
图5.1 用户信息管理页面
### 5.2 课程管理
如图5.2显示的就是课程管理页面，此页面提供给管理员的功能有：查看已发布的课程数据，修改课程，课程作废，即可删除。
...
图5.2 课程管理页面
### 5.3讲师信息管理
如图5.3显示的就是讲师信息管理页面，此页面提供给管理员的功能有：根据讲师信息进行条件查询，还可以对讲师信息进行新增、修改、查询操作等等。
